  ...
  const showNavDropdown = (_) => {
    return (
      <React.Fragment>
        ...
        <NavDropdown
          title={(person && person.name()) || username || "User"}
          id="nav-dropdown"
        >
      ...
      >
        <Container className={`bg-white ${state.isAuth ? "shadow-md-up" : ""}`}>
          ...
          <Navbar.Collapse id="responsive-navbar-nav">
            <Nav className="mr-auto link-border-left" defaultActiveKey="2">
              {showMenu()}
            ...
            <Navbar>{state.isAuth ? showNavDropdown() : ""}</Navbar>
          </Navbar.Collapse>
        </Container>
      </Navbar>
